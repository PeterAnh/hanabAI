...
/**
 * An implentation of Van den Bergh et al agent for the Hanabi card game 
...
public class Agent20163079 implements Agent {
	...
	public Action doAction(State s)
	{
		...
		try{
			...
			if(hintTokens > 0)
			{
				a = tellAnyoneAboutUselessCard(s);
				if(a != null) return a;
			}
			
			if(hintTokens > 0)
			{
				a = TellMostInformation(s);
				if(a != null) return a;
			}
			
			if(hintTokens < 8)
			{
				a = DiscardProbablyUselessCard(s, 0.0);
				if(a != null) return a;
			}
	 ...
	 * likely to be useless if it is at least as probable as Threshold.
	 * @param s The current State.
	 * @param probability A card safe probability must be equal or higher than the threshold.
	 * @return A PLAY action with a card that is probably safe to play or null if no such card is found.
	 */
	public Action DiscardProbablyUselessCard(State s, double probability) throws IllegalActionException
	{
		double highestDiscardability = 0;
		int cardToPlay = -1;
	 ...
	 * with a useless card either the remaining unknown suit of
	 * the card or the rank of the card.
	 * @param s The current State.
	 * @return A HINT_COLOUR or HINT_VALUE which would tell a player (who our agent determines to have useless card)
	 * the colour or value of the useless card they have or null if no one has any useless cards that we can give hint
	 * or there is no hint token left.
	 * @throws IllegalActionException
	 */
	public Action tellAnyoneAboutUselessCard(State s) throws IllegalActionException
	{
		for(int player = (s.getObserver()+1) % numPlayers; player != s.getObserver(); player = (player+1) % numPlayers)
		{
			...
			for(int card=0; card<playerHand.length; card++)
			{
				if(playerHand[card] == null)
				{
					continue;
				}
				if(!isCardDiscardable(s, playerHand[card].getValue(), playerHand[card].getColour(), player) || playerHistory[card].isCardKnown())
				{
					continue;
				} 
				if(!playerHistory[card].isCardUnknown())
	...
	/**
	 * TellMostInformation: Tells whatever reveals
	 * the most information, whether this is the
	 * most information in total or the most new information.
	 * @param s The current State.
	 * @return A HINT_COLOUR or HINT_VALUE which would tell a player
	 * the colour or value of the card which has the most new information or null if no one has any useful 
	 * card that we can give hint or there is no hint token left.
	 * @throws IllegalActionException
	 */
	public Action TellMostInformation(State s) throws IllegalActionException
	{
		int maxPlayer = 0;
		int maxHints = 0;
		int maxCard = 0;
		...
		for(int player = (s.getObserver()+1) % numPlayers; player != s.getObserver(); player = (player+1) % numPlayers)
		{
			...
			for(int card=0; card<playerHand.length; card++)
			{
				if(playerHand[card]==null || playerHistory[card].isCardKnown())
				{
					continue;
				} 
				if(!playerHistory[card].isCardUnknown())
	 ...
	 * @return True if the card is discardable, false otherwise.
	 */
	public boolean isCardDiscardable(State s, int number, Colour colour, int player) 
	{
		int value = 0;
		if(!s.getFirework(colour).isEmpty())
		{
			value = s.getFirework(colour).peek().getValue();
		}
		if(number < value || isCardDisconnected(value, number, colour, player))
		{
			return true;
		}
		return false;
